...
| **API 管理** | **聊天节点 / 图像节点双模式**、聊天节点编辑（`openai (completions)` / `openai-response` / `Anthropic` / `Gemini`、xAI Web Search 开关、URL 默认后缀辅助、拖拽排序、批量测试、健康分、导入/导出）、图像节点编辑（独立 `image_api_config.json`、provider、生成/修图能力、主生成/修图 URL、备用生成/修图 URL 列表、默认比例/分辨率、xAI / OpenAI 模板导出、导入/导出、活跃图像节点切换） |
