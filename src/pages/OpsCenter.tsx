...
export function OpsCenter() {
  ...
  async function doCreateSnapshot() {
    ...
    try {
      ...
      addToast('success', `已创建快照 ${id}，接下来放心改，出问题能退回这里。`);
      await loadAll();
    } catch (e: any) {
  ...
  async function doRollback(snapshotId: string) {
    ...
    try {
      ...
      addToast('success', `已回滚到快照 ${snapshotId}，去概览或配置页确认下状态对不对。`);
    } catch (e: any) {
  ...
  async function doDiff() {
    ...
    try {
      ...
      setDiffResult(diff);
      addToast('success', `对比完成，${diff.changed_files.length} 个文件有变化，看清楚再决定要不要回滚。`);
    } catch (e: any) {
  ...
  async function doExportPackage() {
    ...
    try {
      ...
      addToast('success', `部署包已导出：${r.package_name}，分享前再检查下里面有没有敏感文件。`);
      await loadAll();
    } catch (e: any) {
  ...
  async function doPreviewEnv(env: 'dev' | 'test' | 'prod') {
    ...
    try {
      ...
      addToast('success', `${env} 模板预计改动 ${preview.changed_files.length} 个文件，确认无误再应用。`);
    } catch (e: any) {
  ...
  async function doApplyEnv(env: 'dev' | 'test' | 'prod') {
    ...
    try {
      ...
      addToast('success', `已应用 ${env} 模板，并自动存了一个快照，结果不对就回滚回去。`);
      await loadAll();
    } catch (e: any) {
  ...
  async function doSelfCheck() {
    ...
    try {
      ...
      setSelfCheck(report);
      addToast(report.ok ? 'success' : 'warning', report.ok ? '自检通过，没发现明显问题。' : '自检查出要处理的问题，看下面的说明。');
      await loadAll();
    } catch (e: any) {
  ...
  async function doFixSelfCheck() {
    ...
    try {
      ...
      addToast('success', changed.length > 0 ? `已自动修复 ${changed.length} 项，再跑一次自检确认结果。` : '没有可自动修复的项。');
      if (selfCheck) {
        await doSelfCheck();
      }
    } catch (e: any) {
  ...
  return (
    <div className="space-y-4">
      {lowPerformanceMode ? (
        <Panel title="快速概览" subtitle="低性能模式只显示关键数字，要用工具时再展开下面各区。" icon="⚡" padding="sm">
        ...
        <div className="grid grid-cols-2 xl:grid-cols-5 gap-4">
          ...
          <SummaryCard label="当前说明" value="先看再动" hint="顺手的顺序：先建快照、看差异，再回滚、套模板或自动修复。" />
        </div>
      )}
      ...
      <div className="rounded-[var(--radius)] p-4 border border-[var(--error-soft-border)]" style={{ background: 'var(--error-soft-bg)' }}>
        ...
        <p className="text-xs text-[var(--text-secondary)] leading-relaxed">
          快照和部署包里一般带着 <span className="mono">api_config.json</span>，里面是你的完整 API Key。
          除非真打算把这些 Key 一并交给对方，分享前先删掉 <span className="mono">api_config.json</span>。
          Key 一旦外泄，别人就能直接拿去用，到时只能去各平台改 Key 止损。
        </p>
      </div>
      ...
      <div className={`grid grid-cols-1 ${densityGap}`}>
        <OpsPanel title="快照中心" subtitle="改动前先存个快照，相当于一颗后悔药——出问题能一键退回原样。" icon="📸">
          <PanelToggleRow
            expanded={showSnapshotDetails}
            onToggle={() => setShowSnapshotDetails((value) => !value)}
            collapsedText={`已有 ${snapshots.length} 个快照，最新 ${latestSnapshot === '-' ? '-' : latestSnapshot.slice(-8)}。要对差异或回滚时再展开。`}
          />
          {showSnapshotDetails && (
            <DeferredVisibleBlock placeholder={<OpsCollapsedHint text="滚动到这里时再加载快照差异工具。" />}>
                    ...
                    {diffResult && (
                      <div className="rounded-[var(--radius-sm)] border border-[var(--border-subtle)] bg-[var(--surface-card)] p-3 text-xs space-y-2">
                        ...
                        {diffResult.changed_files.length === 0 ? (
                          <p className="text-[var(--text-muted)]">两个快照内容一致，没有差异。</p>
                        ) : (
      ...
      {subTab === 'deploy' && (
      <div className={`grid grid-cols-1 ${densityGap}`}>
        <OpsPanel title="部署包与环境模板" subtitle="把当前配置打包带走，或存成 dev / test / prod 三套环境模板随时切换。" icon="📦">
          <PanelToggleRow
            expanded={showEnvDetails}
            onToggle={() => setShowEnvDetails((value) => !value)}
            collapsedText="导出部署包，或保存 / 预览 / 应用 dev、test、prod 三套环境模板。低性能模式下默认收起。"
          />
          {showEnvDetails && (
            <DeferredVisibleBlock placeholder={<OpsCollapsedHint text="滚动到这里时再加载部署包与模板工具。" />}>
              <div className="space-y-3">
                <div className="flex flex-wrap items-center gap-2">
      ...
      <div className={`grid grid-cols-1 xl:grid-cols-2 ${densityGap}`}>
        <OpsPanel title="启动前自检" subtitle="先扫一遍常见问题，看清说明再决定要不要自动修复。" icon="🩺">
          <PanelToggleRow
            expanded={showSelfCheckDetails}
            onToggle={() => setShowSelfCheckDetails((value) => !value)}
            collapsedText={selfCheck ? `最近一次自检共 ${checkStats.total} 项，错误 ${checkStats.errors}，警告 ${checkStats.warns}，可修 ${checkStats.fixable}` : '还没跑过自检。改配置、分享部署包或回滚前先跑一次更稳。'}
          />
          {showSelfCheckDetails && (
            <DeferredVisibleBlock placeholder={<OpsCollapsedHint text="滚动到这里时再加载自检明细。" />}>
              <div className="space-y-3">
                <div className="flex flex-wrap items-center gap-2">
                ...
                {selfCheck ? (
                  <div className="space-y-3 rounded-[var(--radius-sm)] border border-[var(--border-subtle)] bg-[var(--bg-elevated)] p-3">
                    ...
                    <div className="max-h-44 overflow-y-auto border border-[var(--border-subtle)] rounded-[var(--radius-sm)] p-2 bg-[var(--surface-card)]">
                      {filteredSelfCheckItems.length === 0 ? (
                        <p className="text-xs text-[var(--text-muted)]">{selfCheck.items.length === 0 ? '无异常项' : '没有符合该筛选的自检项'}</p>
                      ) : (
                  ...
                  <p className="text-xs text-[var(--text-muted)]">还没跑过自检。改配置、分享部署包或回滚前先跑一次更稳。</p>
                )}
              </div>
            </DeferredVisibleBlock>
          )}
        ...
        <OpsPanel title="操作审计日志" subtitle="回看最近做过哪些操作，不用逐条抠技术细节。" icon="🧾">
          <PanelToggleRow
            expanded={showAuditDetails}
            onToggle={() => setShowAuditDetails((value) => !value)}
            collapsedText={`最近 ${auditRows.length} 条记录。低性能模式下默认收起长列表，需要时再展开。`}
          />
          {showAuditDetails && (
            <DeferredVisibleBlock placeholder={<OpsCollapsedHint text="滚动到这里时再加载审计日志。" />}>
              <div className="rounded-[var(--radius-sm)] border border-[var(--border-subtle)] bg-[var(--bg-elevated)] p-3">
                ...
                <div className="max-h-64 overflow-y-auto border border-[var(--border-subtle)] rounded-[var(--radius-sm)] p-2 space-y-1 bg-[var(--surface-card)]">
                  {auditRows.length === 0 ? (
                    <p className="text-xs text-[var(--text-muted)]">还没有操作记录。创建快照、跑自检或应用模板后，这里会逐渐记上。</p>
                  ) : (
